...
### [2026-08-01] Claude Code — 요약본을 원문처럼 인용

- 시도: 과제 안내문 PDF를 `docs/ASSIGNMENT_BRIEF.md`로 요약해 두고, 이후 README·PLAN·DECISIONS를 그 요약본만 보고 작성
- 결과: 수정 필요 — README에 `"실행 직후 로그인 없이 바로 확인"이 제출 요건`이라고 **따옴표를 붙여** 썼는데, 원문은 `실행 직후 로그인 없이도 서비스를 바로 확인할 수 있는 최소 콘텐츠`였다. "없이도"를 "없이"로 줄이면서 **"로그인 벽에 막히지 않아야 한다"가 "로그인이 있으면 안 된다"로 강해졌다.** 사용자가 "이 문구가 실제로 안내문에 있나?"라고 묻지 않았으면 그대로 제출됐다
- 배운 것: 요약은 요약이지 인용이 아니다. **한 번 요약한 문장이 따옴표를 달고 다시 인용되면 출처가 세탁된다** — 이 오인용은 README·PLAN·DECISIONS·QA 4개 파일 6곳으로 이미 번져 있었다. 원문 PDF가 로컬에 있었는데도 요약본만 참조한 것이 실수였다. 이후 원문을 pypdf로 추출해 대조했고, 나머지 인용(`결제·배송·회원 인증은 구현 대상이 아닙니다`, 시크릿·`.gitignore` 요건, README 7섹션 구성)은 정확했다. **채용 과제처럼 원문 준수가 평가 대상인 문서에서는, 따옴표를 칠 때마다 원문을 다시 열어야 한다.**
